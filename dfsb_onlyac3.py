...
def select_unassigned_variable(assignment,csp):
    ...
    # for var in list_unassigned:   # to return maximum degree bounded var
...
def revise(csp,top_element):
    ...
    if(len(csp.domain_values[top_element[1]])==1):
        if(csp.domain_values[top_element[1]][0] in csp.domain_values[top_element[0]] ):
            ...
            print("curr domain of" , top_element[0] ,"is",csp.domain_values[top_element[0]])
            revised=True
    return revised
...
def backtrack(assignment,csp):
    ...
    # domain=order_domain_values(var,csp)
    # print("domain of variable is",domain)#gets the domain of the variable
    # neigh_var=csp.neighbors[var]
    # print('neighboring variables ',neigh_var)
    # lcv={}    #least constraining value dictionary - value : number of times value appears in the domains of neighbors
    ...
    for value in sorted_domain:
        ...
        if consistent_assignment(csp,var,value,assignment)==0:
            ...
            #csp_orig = copy.deepcopy(csp)  #maintainig original copy to restore in case inference fails
            #orig_domain_values_var=copy.deepcopy(csp.domain_values[var])
            ...
            if(inferences==True):
                ...
                if result is not None:
                    return result
            #csp.domain_values[var]=orig_domain_values_var
